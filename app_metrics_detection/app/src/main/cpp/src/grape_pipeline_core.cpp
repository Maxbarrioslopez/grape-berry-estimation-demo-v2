...
PipelineResult GrapePipelineCore::Run(
    ...
    try {
        ...
        qty_inputs.push_back(Ort::Value::CreateTensor<int64_t>(
            memory_info,
            inputs.variety_idx.data(),
            inputs.variety_idx.size(),
            scalar_shape.data(),
            scalar_shape.size()));
        if (contract_.uses_seg_count_base) {
            qty_inputs.push_back(Ort::Value::CreateTensor<float>(
                memory_info,
                inputs.seg_count_base.data(),
                inputs.seg_count_base.size(),
                scalar_shape.data(),
                scalar_shape.size()));
        }

        std::vector<std::string> qty_input_names = {
            contract_.qty_x_input_name,
            contract_.qty_variety_input_name,
        };
        if (contract_.uses_seg_count_base) {
            qty_input_names.push_back(contract_.qty_seg_count_input_name);
        }

        const auto qty_t0 = Clock::now();
        const std::vector<float> qty_output = RunFloatVectorModel(
            qty_session_,
            qty_input_names,
            qty_inputs,
            contract_.qty_output_name);
        const auto qty_t1 = Clock::now();
        result.timing.qty_ms = std::chrono::duration<double, std::milli>(qty_t1 - qty_t0).count();
        if (qty_output.empty()) {
            throw std::runtime_error("La salida QTY vino vacia.");
        }
        ...
        const std::vector<std::string> hist_input_names = {
            contract_.hist_x_input_name,
            contract_.hist_variety_input_name,
        };

        const auto hist_t0 = Clock::now();
        result.hist_prob = NormalizeHistogram(RunFloatVectorModel(
            hist_session_,
            hist_input_names,
            hist_inputs,
            contract_.hist_output_name));
        ...
        if (result.hist_prob.empty()) {
            throw std::runtime_error("La salida HIST vino vacia.");
        }
        ...
        if (peak_it != result.hist_prob.end()) {
            ...
            if (peak_index < result.bins.size()) {
                result.peak_bin_mm = result.bins[peak_index];
            }
        }
